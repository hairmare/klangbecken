...
class APITestCase(unittest.TestCase):
    def setUp(self):
        from klangbecken_api import KlangbeckenAPI, FileAddition, MetadataChange
    ...
    def testUpdate(self):
        ...
        self.assertEqual(
            json.loads(six.text_type(resp.data, "ascii")), {"status": "OK"}
        )
...
class AuthTestCase(unittest.TestCase):
    ...
    def _testLogin(self, method):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(
            json.loads(six.text_type(resp.data, "ascii")),
            {"status": "OK", "user": "xyz"},
        )
    ...
    def testAuthorization(self):
        ...
        self.assertEqual(resp.status_code, 200)
        resp = self.client.post(
            "/music/", data={"file": (io.BytesIO(b"xyz"), "test.mp3")}
        )
        self.assertEqual(resp.status_code, 200)
        resp = self.client.put(
            "/jingles/" + str(uuid.uuid4()) + ".mp3", data=json.dumps({})
        )
        self.assertEqual(resp.status_code, 200)
        resp = self.client.delete("/music/" + str(uuid.uuid4()) + ".ogg")
        self.assertEqual(resp.status_code, 200)
...
class AnalyzersTestCase(unittest.TestCase):
    ...
    def testRawFileAnalyzer(self):
        import time
        from klangbecken_api import raw_file_analyzer, FileAddition, MetadataChange
...
class ProcessorsTestCase(unittest.TestCase):
    ...
    def testCheckProcessor(self):
        ...
        with self.assertRaises(UnprocessableEntity) as cm:
            check_processor(
                self.tempdir, "playlist", "id", ".ext", [MetadataChange("count", "1")]
            )
    ...
    def testPlaylistProcessor(self):
        ...
        playlist_processor(
            self.tempdir, "music", "fileId1", ".mp3", [MetadataChange("count", 2)]
        )
        ...
        playlist_processor(
            self.tempdir, "music", "fileId1", ".mp3", [MetadataChange("count", 4)]
        )
        ...
        playlist_processor(
            self.tempdir, "music", "fileId1", ".mp3", [MetadataChange("count", 0)]
        )
        ...
        playlist_processor(
            self.tempdir, "music", "fileId2", ".ogg", [MetadataChange("count", 1)]
        )
        playlist_processor(
            self.tempdir, "music", "fileId3", ".flac", [MetadataChange("count", 3)]
        )
        playlist_processor(
            self.tempdir, "jingles", "fileId4", ".mp3", [MetadataChange("count", 2)]
        )
        playlist_processor(
            self.tempdir, "jingles", "fileId5", ".mp3", [MetadataChange("count", 3)]
        )
...
class StandaloneWebApplicationStartupTestCase(unittest.TestCase):
    ...
    def testNoFFmpegWarning(self):
        ...
        with capture(StandaloneWebApplication, self.tempdir) as (out, err, ret):
...
class StandaloneWebApplicationTestCase(unittest.TestCase):
    ...
    def testApi(self):
        ...
        with open(path, "rb") as f:
            resp = self.client.post(
                "/api/music/", data={"file": (f, "silence-unicode-jointstereo.mp3")}
            )
        self.assertEqual(resp.status_code, 200)
        data = json.loads(six.text_type(resp.data, "ascii"))
        fileId = list(data.keys())[0]
    ...
    def testImport(self):
        ...
        try:
            ...
            with open(os.path.join(self.tempdir, "index.json")) as file:
                ...
                self.assertEqual(
                    list(data.values())[0]["import_timestamp"], audio1_mtime
                )
                self.assertEqual(
                    list(data.values())[0]["original_filename"], "silence.mp3"
                )
