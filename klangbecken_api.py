...
TAG_KEYS = (
    "artist title album cue_in cue_out track_gain "
    "original_filename import_timestamp count"
).split()
...
def mutagen_tag_analyzer(playlist, fileId, ext, file_):
    ...
    try:
        ...
        raise UnprocessableEntity("Unsupported file type: " + "Cannot read metadata.")
    changes = [
...
def update_data_analyzer(playlist, fileId, ext, data):
    ...
    for key, value in data.items():
        if key not in UPDATE_KEYS:
            raise UnprocessableEntity(
                "Invalid data format: " + "Key not allowed: " + key
            )
    ...
    for change in changes:
        if isinstance(change, MetadataChange):
            ...
            if key not in ALLOWED_METADATA.keys():
                raise UnprocessableEntity("Invalid metadata key: {0}".format(key))
            ...
            for check in checks:
                if isinstance(check, type):
                    if not isinstance(val, check):
                        raise UnprocessableEntity(
                            'Invalid data format for "{0}": Type error '
                            "(expected {1}, got {2}).".format(
                                key, check.__name__, type(val).__name__
                            )
                        )
                elif callable(check):
                    if not check(val):
                        raise UnprocessableEntity(
                            'Invalid data format for "{0}": Check failed '
                            '(value: "{1}").'.format(key, val)
                        )
                elif isinstance(check, text_type):
                    if re.match(check, val) is None:
                        raise UnprocessableEntity(
                            'Invalid data format for "{0}": Regex check failed '
                            '(value: "{1}", regex: "{2}").'.format(key, val, check)
                        )
                else:
...
class KlangbeckenAPI:
    ...
    def on_update(self, request, playlist, fileId, ext):
        ...
        try:
            ...
            raise UnprocessableEntity(
                "Cannot parse PUT request: " + " not valid UTF-8 data"
            )
        except ValueError:
            raise UnprocessableEntity("Cannot parse PUT request: " + " not valid JSON")
...
class StandaloneWebApplication:
    ...
    * Relays API calls to the KlangbeckenAPI instance
    ...
    def __init__(self, data_path=None):
        ...
        app = SharedDataMiddleware(app, {"": dist_full_path, "/data": data_full_path})
    ...
    for path in [data_dir] + [
        ...
        if not os.path.isdir(path):
            if create:
                ...
                raise Exception('Directory "{0}" does not exist'.format(path))
    for path in [os.path.join(data_dir, d + ".m3u") for d in PLAYLISTS + ("prio",)]:
        if not os.path.isfile(path):
            if create:
                ...
                raise Exception('Playlist "{0}" does not exist'.format(path))
...
# Entry points #
################
def import_files(interactive=True):  # noqa: C901
    """Entry point for import script."""

    def err(*args):
        print(
            *(
                arg if isinstance(arg, text_type) else text_type(arg, "utf-8")
                for arg in args
            ),
            file=sys.stderr
        )
    ...
    if playlist not in PLAYLISTS:
        err("ERROR: Invalid playlist name: {0}".format(playlist))
        sys.exit(1)

    analysis_data = []
    for filename in files:
        try:
            analysis_data.append(_analyze_one_file(data_dir, playlist, filename))
        except UnprocessableEntity as e:
            err("WARNING: File cannot be analyzed: " + filename)
            err(
                "WARNING: " + e.description
    ...
    print(
        "Successfully analyzed {0} of {1} files.".format(len(analysis_data), len(files))
    )
    count = 0
        ...
        for filename, fileId, ext, actions in analysis_data:
            try:
                ...
                err(
                    ...
                    e.description if hasattr(e, "description") else text_type(e),
                )

    print("Successfully imported {0} of {1} files.".format(count, len(files)))
...
def _analyze_one_file(data_dir, playlist, filename):
    ...
    with open(filename, "rb") as importFile:
        ...
        actions.append(MetadataChange("import_timestamp", os.stat(filename).st_mtime))
            ...
            if not os.path.isfile(file_full_path):
                ...
                for key in TAG_KEYS:
                    ...
                    if text_type(entries[key]) != tag_value:
                        err(
                            'ERROR: Tag value mismatch "{0}": {1} != {2}'.format(
                                key, entries[key], tag_value
                            )
                        )
                ...
                if count != entries["count"]:
                    err(
                        "ERROR: Playlist count mismatch: {0} != {1}".format(
                            entries["count"], count
                        )
                    )
